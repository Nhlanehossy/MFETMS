...
export default function PublicMatchDetails() {
  ...
  if (!match) {
    return <EmptyState title="Match not found" message="The selected fixture is not available in the public calendar." />;
  }
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {match.ticket_prices.map((price) => (
          <Card key={price.id} className="p-5">
            ...
            <Link to="/login" state={{ from: `/supporter/checkout?match=${match.id}&category=${price.ticket_category?.id}` }}>
              <Button className="mt-4 w-full">Login to Buy</Button>
            </Link>
          </Card>
        ))}
      </section>
